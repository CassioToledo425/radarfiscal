...
UA = "RadarFiscalGitHub/1.1 (+https://github.com/CassioToledo425/radarfiscal)"
...
SOURCES = [
    ...
    {"name": "Receita Federal — Reforma Tributária", "level": "official",
     "url": "https://www.gov.br/receitafederal/pt-br/acesso-a-informacao/acoes-e-programas/programas-e-atividades/reforma-tributaria-do-consumo/rtc2026",
...
def meaningful_links(page_soup: BeautifulSoup, base_url: str, require_keywords=False):
    ...
    for a in page_soup.find_all("a", href=True):
        ...
        if parsed.scheme not in ("http", "https"):
            continue
        if host not in parsed.netloc.replace("www.", ""):
            continue
        low = f"{title} {url}".lower()
        if any(x in low for x in (
            "facebook", "linkedin", "instagram", "youtube", "whatsapp",
            "política de", "termos de", "mapa do site"
        )):
            continue
...
def accessory_group(description: str) -> str:
    """Produz um rótulo curto para declarações/obrigações acessórias."""
    head = re.split(r"\s+[–—-]\s+", clean(description), maxsplit=1)[0].strip()
    if head and len(head) <= 46:
        return head
    return "Obrigação acessória"


def parse_agenda_table(table, iso: str, day_url: str):
    """Converte uma tabela da página diária da Receita em eventos do calendário."""
    rows = table.find_all("tr")
    if not rows:
        return []

    header = clean(rows[0].get_text(" ", strip=True)).lower()
    events = []

    if "código de receita" in header or "codigo de receita" in header or "grupo de tributo" in header:
        for tr in rows[1:]:
            ...
            if not any((code, group, desc)):
                continue
            events.append({
                "date": iso,
                "kind": "payment",
                "code": code,
                "group": group or "Tributo federal",
                "description": desc,
                "period": period,
                "document": document,
                "category": category,
    ...
    if (
        "declarações" in header
        or "declaracoes" in header
        or "demonstrativos e documentos" in header
        or "interessado" in header
    ):
        for tr in rows[1:]:
            cells = [clean(c.get_text(" ", strip=True)) for c in tr.find_all(["td", "th"])]
            if not cells:
                continue
            cells += [""] * (4 - len(cells))
            interested, desc, period, legal = cells[:4]
            if not desc:
                continue
            events.append({
                "date": iso,
                "kind": "accessory",
                "group": accessory_group(desc),
                "description": desc,
                "period": period,
...
def collect_agenda_month(y: int, m: int):
    ...
    for day_url in links:
        ...
        try:
            ...
            day_events = []
            for table in tables:
                day_events.extend(parse_agenda_table(table, iso, day_url))
            ...
            if day_events:
                ...
                events.append({
                    "date": iso,
                    "kind": "unknown",
                    "group": "Agenda Receita Federal",
                    "description": "Há item(ns) na agenda oficial; consulte o detalhamento na fonte.",
                    "url": day_url,
                })
            ...
            events.append({
                ...
                "kind": "unknown",
                "group": "Agenda Receita Federal",
                "description": "Detalhamento temporariamente indisponível na coleta; consulte a fonte oficial.",
                "url": day_url,
            })
...
def collect_source(
    ...
    for row in links[: limit * 3]:
        title = row["title"]
        if title.lower() in {"reforma tributária", "notícias", "tributação", "serviços", "institucional"}:
            continue
        items.append({
...
def merge_with_previous(name: str, new_items: list, limit: int) -> list:
    """Evita apagar o último conteúdo válido se uma fonte renderizar pouco/zero HTML."""
    previous = load_json(name, [])
    combined = [*new_items, *previous]
    out, seen = [], set()
    for x in combined:
        key = (x.get("url") or "").rstrip("/") or (x.get("title") or "").lower()
...
def collect_reforma():
    ...
    configs = [
        ("Receita Federal",
         "https://www.gov.br/receitafederal/pt-br/acesso-a-informacao/acoes-e-programas/programas-e-atividades/reforma-tributaria-do-consumo/rtc2026",
         "official", "update", "Reforma Tributária", False, 20),
        ("Receita Federal",
         "https://www.gov.br/receitafederal/pt-br/assuntos/noticias/ultimas-noticias",
         "official", "update", "Notícia oficial", True, 20),
        ("CGIBS",
         ...
         "official", "update", "Notícia oficial", True, 20),
        ("CGIBS", "https://www.cgibs.gov.br/atos-conjuntos",
         "official", "normative", "Ato conjunto", False, 15),
        ("CGIBS", "https://www.cgibs.gov.br/resolucoes",
         "official", "normative", "Resolução", False, 15),
    ]
    for cfg in configs:
        try:
            got = collect_source(*cfg)
            items.extend(got)
            print(f"[reforma] {cfg[0]} {cfg[4]}: {len(got)}")
        except Exception as exc:
            print(f"[reforma] {cfg[0]}: {exc}", file=sys.stderr)

    out = merge_with_previous("reforma.json", items, 60)
    if out:
        write_json("reforma.json", out)


def collect_radar():
    items = []
    configs = [
        ("FENACON", "https://fenacon.org.br/reforma-tributaria/",
         "institutional", "update", "Reforma Tributária", False, 20),
        ("CFC", "https://cfc.org.br/search/reforma%20tributaria",
         "institutional", "update", "CFC", False, 15),
        ("Portal Contábeis", "https://www.contabeis.com.br/conteudo/tributario",
         "specialized", "update", "Tributário", True, 25),
    ]
    for cfg in configs:
        try:
            got = collect_source(*cfg)
            for item in got:
                item["category"] = item.pop("kind_label", "Atualização")
                item.pop("kind", None)
            items.extend(got)
            print(f"[radar] {cfg[0]}: {len(got)}")
        except Exception as exc:
            print(f"[radar] {cfg[0]}: {exc}", file=sys.stderr)

    out = merge_with_previous("radar.json", items, 80)
    if out:
        write_json("radar.json", out)
...
def collect_meta():
    ...
    for src in SOURCES:
        ...
        try:
            ...
            status = previous_status.get(src["name"], "error")
